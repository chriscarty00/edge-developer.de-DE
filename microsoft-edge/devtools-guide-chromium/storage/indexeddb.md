...
1.  Wählen Sie die Registerkarte **Anwendung** aus, um das **Anwendungs** Tool zu öffnen.  Der Bereich **Manifest** wird normalerweise standardmäßig geöffnet.  
...
1.  Erweitern Sie das **IndexedDB** -Menü, um zu überprüfen, welche Datenbanken verfügbar sind.  
    ...
    *   **Titel** und **Text** sind [Indizes][MDNUsingIndexedDBUsingIndex].  
    ...
    > [!NOTE]
    > **Bekannte Einschränkung**  Datenbanken von Drittanbietern werden nicht angezeigt.  Wenn Sie beispielsweise eine Anzeige in `<iframe>` Ihre Seite einbetten und Ihr Anzeigennetzwerk IndexedDB verwendet, sind die IndexedDB-Daten für Ihr Anzeigennetzwerk nicht sichtbar.  Navigieren Sie zu [Issue #943770][ChromiumIssue943770].  
...
1.  Wählen Sie einen Objektspeicher aus, um die Schlüssel-Wert-Paare zu überprüfen.  
    ...
    > [!NOTE]
    > IndexedDB-Daten werden in Echtzeit nicht aktualisiert.  Navigieren Sie zu [IndexedDB-Daten aktualisieren](#refresh-indexeddb-data).  
    ...
    *   Der **Schlüsselgenerator Wert** ist der nächste verfügbare Schlüssel.  Das Feld wird nur angezeigt, wenn [Schlüsselgeneratoren][MDNBasicConceptsKeyGenerator]verwendet werden.  
    
1.  Wählen Sie in der Spalte **Wert** eine Zelle aus, um den Wert zu erweitern.  
...
IndexedDB-Werte im **Anwendungs** Tool werden in Echtzeit nicht aktualisiert.  Wählen Sie **Aktualisieren** \ ( ![ Aktualisieren \) aus, wenn Sie ][ImageReloadIcon] einen Objektspeicher anzeigen, um die Daten zu aktualisieren, oder zeigen Sie eine Datenbank an, und wählen Sie **Datenbank aktualisieren** aus, um alle Daten zu aktualisieren.  
...
IndexedDB-Schlüssel und-Werte können vom **Anwendungs** Tool nicht bearbeitet werden.  Da devtools jedoch Zugriff auf den Seitenkontext hat, können Sie JavaScript-Code in devtools ausführen, um IndexedDB-Daten zu bearbeiten.  
