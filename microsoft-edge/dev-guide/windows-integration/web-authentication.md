...
**1. registrieren Sie Ihren Nutzer bei `create`**

**2. authentifizieren Sie Ihren Benutzer mit `get`**
...
Wenn Sie die `create` Methode verwenden, fordert Microsoft Edge zunächst den Benutzer auf, seinen Anwesenheitsstatus zu überprüfen, indem er sein Angesicht oder seinen Fingerabdruck scannt, eine PIN eingibt oder auf einem externen FIDO2-Gerät eine Aktion durchführt. Nach Abschluss dieses Schritts generiert der Authentifikator ein öffentliches/privates Schlüsselpaar und speichert den privaten Schlüssel. Diese Anmeldeinformationen werden pro Herkunft und Konto erstellt und können nicht extrahiert werden, da Sie sicher auf dem Authentifizierungsgerät gespeichert werden. 
...
- Die Kandidaten Empfehlungs Version der Webauthentifizierungs-API kann von Microsoft Edge beginnend mit EdgeHTML 18 (Windows Insider Preview Version 17713 und nach oben) verwendet werden.
- Die vorangestellte [Preview-Version](https://blogs.windows.com/msedgedev/2016/04/12/a-world-without-passwords-windows-hello-in-microsoft-edge/) der Webauthentifizierungs-API wurde entfernt und steht nicht mehr zur Verfügung.
...
- **Windows Hello**, Aktivieren der Kenn Wort losen Authentifizierung auf dem Gerät mit Face, Fingerabdruck oder PIN
- **FIDO2**, Aktivieren der Kenn Wort ungebundenen Roaming-Authentifizierung mit einem Wechselmedium und einem Fingerabdruck oder einer PIN
- **U2F**: Aktivieren der starken zweiten Faktor Authentifizierung für Websites, die nicht bereit sind, zu einem Kenn Wort ungültigen Modell zu wechseln
...
- Windows Hello verwendet verschiedene Möglichkeiten, um Benutzeranmeldeinformationen zu schützen. Sie können überprüfen, welche Methode zum Schützen von Anmeldeinformationen verwendet wurde, indem Sie das [AAGUID](https://w3c.github.io/webauthn/#sec-attested-credential-data) -Feld im Bestätigungs Objekt verwenden, das bei der Erstellung von Anmeldeinformationen zurückgegeben wird. Die folgende Liste enthält die AAGUIDs, die von Windows Hello zurückgegeben werden können: 
  ...
  - Gesicherte Authentifikatoren für Trusted Platform Module (TPM)
