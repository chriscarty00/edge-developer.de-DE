...
---
# Microsoft Edge (Chromium)-Entwicklungstools  
...
Wenn Sie Microsoft Edge ausprobieren und bisher hauptsächlich in einem Browser auf Chromium-Basis entwickelt haben, sollten Sie sich wie zuhause fühlen.  Die Microsoft Edge \(Chromium\)-Entwicklungstools funktionieren auf die gleiche Weise wie die Entwicklungstools, die Sie bereits kennen und verwenden.  Weitere Informationen finden Sie unter [Neuigkeiten in den Microsoft Edge (Chromium)-Entwicklungstools][DevtoolsGuideChromiumWhatsNewIndex].  

:::image type="complex" source="./devtools-guide-chromium/media/devtools.png" alt-text="Microsoft Edge (Chromium)-Entwicklungstools (DevTools)" lightbox="./devtools-guide-chromium/media/devtools.png":::
   Microsoft Edge (Chromium)-Entwicklungstools (DevTools)  
:::image-end:::  

Wenn Sie den neuen Microsoft Edge ausprobieren und bisher in Microsoft Edge \(EdgeHTML\) entwickelt haben, stehen Ihnen nun einige tolle neue Tools zu Verfügung, mithilfe derer Sie Ihre Websites in Microsoft Edge einfacher und schneller erstellen und testen können!  

## Entwicklungstools öffnen  

Falls Sie die Entwicklungstools noch nie verwendet haben: Bei den Microsoft Edge-Entwicklungstools handelt es sich um eine Reihe von Tools, die direkt in den Microsoft Edge-Browser integriert sind.  Mit diesen Entwicklungstools können Sie Folgendes tun.  
...
Und das alles direkt im Browser.  Das sind nur einige Beispiele für die Features, die Ihnen in den Entwicklungstools zur Verfügung stehen, damit Sie einfacher und schneller Ihre Websites in Microsoft Edge erstellen und testen können.  

So öffnen Sie die Entwicklungstools  

*   drücken Sie `F12` 
*   drücken Sie `Ctrl`+`Shift`+`I` auf Windows/Linux \(`Command`+`Option`+`I` auf macOS\)  
...
Wenn Sie Protokolle Ihres Front-End-Codes in JavaScript anzeigen oder schnell ein Skript ausführen möchten, drücken Sie `Ctrl`+`Shift`+`J` auf Windows/Linux oder `Command`+`Option`+`J` auf macOS, um den Bereich „Konsole“ in den Entwicklungstools zu starten.  

## Grundlegende Tools  

:::image type="complex" source="./devtools-guide-chromium/media/devtools-core-tools.png" alt-text="Microsoft Edge (Chromium)-DevTools – Grundlegende Tools" lightbox="./devtools-guide-chromium/media/devtools-core-tools.png":::
...
Wählen Sie oben **Erweiterungen aus anderen Stores** und wählen Sie dann **Zulassen** in dem Dialog, der sich öffnet.  
...
Wählen Sie **Zu Chrome hinzufügen** aus, um Ihre Entwicklungstools-Erweiterung zu Microsoft Edge hinzuzufügen!  
...
| Entwicklungstools im **Modus „Element prüfen“** anzeigen, in dem Sie ein Element auf der Seite auswählen und den HTML und CSS im Bereich **Elemente** ansehen können | `Ctrl`+`Shift`+`C` | `Command`+`Option`+`C` |  
...
| Entwicklungstools in der letzten verwendeten Position andocken.  Wenn die Entwicklungstools die gesamte Sitzung über in der Standardposition bleiben, sorgt diese Tastenkombination dafür, dass sie in einem separaten Fenster abgedockt werden | `Ctrl`+`Shift`+`D` | `Command`+`Shift`+`D` |  
