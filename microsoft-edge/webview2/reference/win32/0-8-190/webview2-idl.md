...
--------------------------------|---------------------------------------------
[CreateWebView2EnvironmentWithDetails](#createwebview2environmentwithdetails) | DLL-Export zum Erstellen einer WebView2-Umgebung mit einer benutzerdefinierten Version von Edge, Benutzerdatenverzeichnis und/oder zusätzlichen Browser-Switches.
[CreateWebView2Environment](#createwebview2environment) | Erstellt eine immergrüne WebView2-Umgebung unter Verwendung der installierten Edge-Version.
[GetWebView2BrowserVersionInfo](#getwebview2browserversioninfo) | Rufen Sie die Browser Versionsinformationen einschließlich des Kanal namens ab, wenn es sich nicht um den stabilen Kanal oder den eingebetteten Edge handelt.
...
Wenn eine Außerkraftsetzungs Umgebungsvariable gefunden wird, verwenden wir die browserExecutableFolder-, userDataFolder-und additionalBrowserArguments-Werte als Ersatz für die entsprechenden Werte in CreateWebView2EnvironmentWithDetails-Parametern.
...
Als erstes überprüfen wir root als HKLM und dann HKCU. Die Anwendungs-ID des aufrufenden Prozesses wird zunächst auf die Anwendungsbenutzer Modell-ID festgesetzt, und wenn kein entsprechender Registrierungsschlüssel vorhanden ist, wird die APP auf den ausführbaren Namen des Prozesses des Aufrufers festgesetzt, oder wenn es sich nicht um einen Registrierungsschlüssel handelt, "*". Wenn ein Registrierungsschlüssel für die Überschreibung gefunden wird, verwenden wir die Registrierungswerte browserExecutableFolder, userDataFolder und additionalBrowserArguments als Ersatz für die entsprechenden Werte in CreateWebView2EnvironmentWithDetails-Parametern. Wenn einer dieser Registrierungswerte nicht vorhanden ist, wird der an CreateWebView2Environment übergebene Parameter verwendet.
...
Dies entspricht dem Aufruf von CreateWebView2EnvironmentWithDetails mit nullptr für browserExecutableFolder, userDataFolder, additionalBrowserArguments. Weitere Informationen finden Sie unter CreateWebView2EnvironmentWithDetails.
...
Kanalnamen sind Beta, dev und Canary. Wenn für die browserExecutableFolder oder die Kanaleinstellung eine Überschreibung vorhanden ist, wird die Außerkraftsetzung verwendet. Wenn keine Überschreibung vorhanden ist, wird der an GetWebView2BrowserVersionInfo übergebene Parameter verwendet.
...
Sie kann verwendet werden, um zu ermitteln, ob webview2 oder eine bestimmte featurebasis auf Version verwendet werden soll. Legt den Wert des Ergebnisses auf-1, 0 oder 1 fest, wenn version1 kleiner als, gleich oder größer als Version2 ist. Gibt E_INVALIDARG zurück, wenn keine der Versionszeichenfolgen analysiert werden kann oder ein Eingabeparameter NULL ist. Die Eingabe kann die von GetWebView2BrowserVersionInfo abgerufene VERSIONINFO direkt verwenden, Kanalinformationen werden ignoriert.
